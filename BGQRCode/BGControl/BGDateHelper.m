...
+ (NSString *)getTimeStringFromNumberTimer:(NSString *)timerStr isMinuteLater:(BOOL)isMinute{
    ...
    if (isMinute) {
        if (timeInterval < 60) {
            ...
            NSInteger timeInt = ([self getTimeStempByString:currentDay havehh:NO].longLongValue - [self getTimeStempByString:theDay havehh:NO].longLongValue)/86400/1000 + 1;
            ...
            if (timeInt == 0) {
                ...
                return [NSString stringWithFormat:@"%ld天前",timeInt];
            }
        }
    }else{
        df.dateFormat = @"MM-dd";
